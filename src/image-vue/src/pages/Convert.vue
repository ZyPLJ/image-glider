          ...
          <div class="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span class="font-medium text-blue-600 dark:text-blue-400">{{ settings.targetFormat.toUpperCase() }}</span>
              </div>
              <div>
                <span class="text-gray-500 dark:text-gray-400">原始大小：</span>
                <span class="font-medium text-gray-700 dark:text-gray-300">{{ formatFileSize(selectedFile.size) }}</span>
              </div>
              <div>
...
const convertImage = async () => {
  ...
  try {
    ...
    // For demo purposes, create converted version using canvas
    ...
    img.onload = () => {
      ...
      let quality = settings.value.quality / 100
      
      if (settings.value.targetFormat === 'png') {
        mimeType = 'image/png'
        quality = undefined // PNG doesn't use quality
      } else if (settings.value.targetFormat === 'webp') {
        mimeType = 'image/webp'
      } else if (settings.value.targetFormat === 'gif') {
        mimeType = 'image/gif'
        quality = undefined
      }
